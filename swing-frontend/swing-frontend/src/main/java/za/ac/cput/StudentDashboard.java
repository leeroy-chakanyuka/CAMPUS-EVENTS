...
public class StudentDashboard extends JFrame {
    ...
    private JButton btnBrowseEvents;
    private JButton btnMyTickets;
    ...
    private JPanel buildNavigation() {
        ...
        btnBrowseEvents = navButton("Browse Events");
        btnMyTickets = navButton("My Tickets");
        btnNotifications = navButton("Notifications");
        btnLogout = navButton("Logout");

        btnBrowseEvents.setEnabled(false);
        btnMyTickets.setEnabled(false);
        ...
        nav.add(btnBrowseEvents);
        nav.add(Box.createVerticalStrut(4));
        nav.add(btnMyTickets);
        nav.add(Box.createVerticalStrut(4));
    ...
    private void setActiveNav(JButton active) {
        for (JButton b : new JButton[]{btnDashboard, btnBrowseEvents, btnMyTickets, btnNotifications}) {
    ...
    private JPanel buildDashboardPanel() {
        ...
        // TODO: GET /event?open=true (count), GET /ticket/student/{id} (count),
        // GET /notification/student/{id}?unread=true (count) — seed values for now.
        JPanel cards = new JPanel(new GridLayout(1, 3, 20, 20));
        cards.setBackground(Color.WHITE);
        cards.add(statCard("6", "Upcoming events"));
        cards.add(statCard("2", "My registrations"));
        cards.add(statCard("3", "Unread notifications"));
